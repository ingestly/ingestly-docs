CREATE OR REPLACE FUNCTION `proj.dataset.channel_group`(source STRING, medium STRING, name STRING, referrer STRING) AS (case
  ...
  when source = '' and regexp_contains(referrer, r'//(www\.google\.[a-z]+(\.[a-z]+)?|yandex\.ru)/$') then 'Organic Search'
  when source = '' and regexp_contains(referrer, r'//www\.google\.[a-z]+(\.[a-z]+)?/(url|search|m)\?') then 'Organic Search'
